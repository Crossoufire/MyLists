...
def upgrade():
    ...
    for row in results:
        ...
        if row.old_status is not None or row.new_status is not None:
            ...
            update_values["payload"] = json.dumps({
                "old_value": format_status(row.old_status),
                "new_value": format_status(row.new_status),
            })
        elif row.old_season is not None or row.new_season is not None:
        ...
        elif row.old_playtime is not None or row.new_playtime is not None:
            update_values["update_type"] = "PLAYTIME"
            update_values["payload"] = json.dumps({"old_value": row.old_playtime, "new_value": row.new_playtime})
        elif row.old_page is not None or row.new_page is not None:
            update_values["update_type"] = "PAGE"
            update_values["payload"] = json.dumps({"old_value": row.old_page, "new_value": row.new_page})
        elif row.old_redo is not None or row.new_redo is not None:
            update_values["update_type"] = "REDO"
            update_values["payload"] = json.dumps({"old_value": row.old_redo, "new_value": row.new_redo})
