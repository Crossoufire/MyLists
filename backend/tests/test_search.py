...
class SearchTests(BaseTest):
    ...
    def test_autocomplete_general(self):
        ...
        rv = self.client.get("/api/autocomplete?q=test&selector=invalid", headers=headers)
        assert rv.status_code == 400

        rv = self.client.get("/api/autocomplete", headers=headers)
        assert rv.status_code == 400

    def test_autocomplete_users(self):
        headers = self.connexion()

        rv = self.client.get("/api/autocomplete?q=test&selector=users", headers=headers)
        ...
        rv = self.client.get("/api/autocomplete?q=bobby&selector=users", headers=headers)
        assert rv.status_code == 200
    ...
    def test_autocomplete_tmdb(self):
        ...
        with mock.patch("backend.api.routes.search.ApiTMDB.create_search_results") as mock_search:
            ...
            rv = self.client.get("/api/autocomplete?q=friends&selector=TMDB", headers=headers)
            assert rv.status_code == 200
        ...
        with mock.patch("backend.api.routes.search.ApiTMDB.create_search_results") as mock_search:
            ...
            rv = self.client.get("/api/autocomplete?q=toto&selector=TMDB", headers=headers)
            assert rv.status_code == 200
            assert len(rv.json["data"]["items"]) == 0
            assert rv.json["data"]["total"] == 0
            assert rv.json["data"]["pages"] == 1
    ...
    def test_autocomplete_igdb(self):
        ...
        with mock.patch("backend.api.routes.search.ApiGames.create_search_results") as mock_search:
            ...
            rv = self.client.get("/api/autocomplete?q=halo&selector=IGDB", headers=headers)
            assert rv.status_code == 200
            assert len(rv.json["data"]["items"]) == 1
        ...
        with mock.patch("backend.api.routes.search.ApiGames.create_search_results") as mock_search:
            ...
            rv = self.client.get("/api/autocomplete?q=toto&selector=IGDB", headers=headers)
            assert rv.status_code == 200
            assert len(rv.json["data"]["items"]) == 0
            assert rv.json["data"]["total"] == 0
            assert rv.json["data"]["pages"] == 1
    ...
    def test_autocomplete_books(self):
        ...
        with mock.patch("backend.api.routes.search.ApiBooks.create_search_results") as mock_search:
            ...
            rv = self.client.get("/api/autocomplete?q=harry%20potter&selector=BOOKS", headers=headers)
            assert rv.status_code == 200
            assert len(rv.json["data"]["items"]) == 1
        ...
        with mock.patch("backend.api.routes.search.ApiBooks.create_search_results") as mock_search:
            ...
            rv = self.client.get("/api/autocomplete?q=toto&selector=BOOKS", headers=headers)
            assert rv.status_code == 200
            assert len(rv.json["data"]["items"]) == 0
            assert rv.json["data"]["total"] == 0
            assert rv.json["data"]["pages"] == 1
