...
class AuthTests(BaseTest):
    def _get_token(self):
        rv = self.client.post("/api/auth/tokens", auth=(TEST_USER["username"], TEST_USER["password"]))
        self.assertEqual(rv.status_code, 200)
        return rv

    def test_no_auth(self):
        rv = self.client.get("/api/user/me")
        self.assertEqual(rv.status_code, 401)

    def test_no_login(self):
        rv = self.client.post("/api/auth/tokens")
        self.assertEqual(rv.status_code, 401)

    def test_bad_login(self):
        rv = self.client.post("/api/auth/tokens", auth=(TEST_USER["username"], "bad-password"))
        self.assertEqual(rv.status_code, 401)
    ...
    def test_get_token(self):
        ...
        rv = self.client.get("/api/user/me", headers={"Authorization": f"Bearer {access_token}"})
        self.assertEqual(rv.status_code, 200)
        self.assertEqual(rv.json["username"], TEST_USER["username"])

        rv = self.client.get("/api/user/me", headers={"Authorization": f'Bearer {access_token + "x"}'})
        self.assertEqual(rv.status_code, 401)

        rv = self.client.get("/api/user/me", headers={"Authorization": f"Bearer {refresh_token}"})
        self.assertEqual(rv.status_code, 401)
    ...
    def test_token_expired(self):
        ...
        with mock.patch("backend.api.models.users.datetime") as dt:
    ...
    def test_refresh_token(self):
        ...
        rv = self.client.put("/api/auth/tokens", json={"access_token": access_token1})
        self.assertEqual(rv.status_code, 200)
        ...
        rv = self.client.get("/api/user/me", headers={"Authorization": f"Bearer {access_token2}"})
        self.assertEqual(rv.status_code, 200)
        self.assertEqual(rv.json["username"], TEST_USER["username"])

        rv = self.client.get("/api/user/me", headers={"Authorization": f"Bearer {access_token1}"})
        self.assertEqual(rv.status_code, 401)
    ...
    def test_refresh_token_failure(self):
        ...
        self.client.delete_cookie("refresh_token", path="/api/auth/tokens")
        rv = self.client.put("/api/auth/tokens", json={"access_token": access_token})
        self.assertEqual(rv.status_code, 401)
    ...
    def test_revoke(self):
        ...
        rv = self.client.get("/api/user/me", headers=header)
        self.assertEqual(rv.status_code, 200)

        rv = self.client.delete("/api/auth/logout", headers=header)
    ...
    def test_refresh_revoke_all(self):
        ...
        self.client.set_cookie("refresh_token", refresh_token2, path="/api/auth/tokens")
        rv = self.client.put("/api/auth/tokens", json={"access_token": access_token2})
        self.assertEqual(rv.status_code, 200)
        ...
        rv = self.client.get("/api/user/me", headers={"Authorization": f"Bearer {access_token3}"})
        self.assertEqual(rv.status_code, 401)

        self.client.set_cookie("refresh_token", refresh_token1, path="/api/auth/tokens")
        rv = self.client.put("/api/auth/tokens", json={"access_token": access_token1})
        self.assertEqual(rv.status_code, 401)
        ...
        self.client.set_cookie("refresh_token", refresh_token3, path="/api/auth/tokens")
        rv = self.client.put("/api/auth/tokens", json={"access_token": access_token3})
        self.assertEqual(rv.status_code, 401)

    def test_oauth(self):
        rv = self.client.get("/api/auth/oauth2/bar")
        self.assertEqual(rv.status_code, 400)

        rv = self.client.get("/api/auth/oauth2/bar?callback=http://localhost:3000/oauth2/bar/callback")
        self.assertEqual(rv.status_code, 404)

        rv = self.client.get("/api/auth/oauth2/foo?callback=http://localhost:3000/oauth2/foo/callback")
        self.assertEqual(rv.status_code, 200)

        redirect_url = rv.json["redirect_url"]
        args = redirect_url.split("?")[1].split("&")

        self.assertTrue(redirect_url.startswith("https://foo.com/login?"))
        self.assertTrue("client_id=foo-id" in args)
        self.assertTrue("redirect_uri=http%3A%2F%2Flocalhost%3A3000%2Foauth2%2Ffoo%2Fcallback" in args)
        self.assertTrue("scope=user+email" in args)
        self.assertTrue("response_type=code" in args)
        ...
        with mock.patch("backend.api.routes.auth.requests.post") as requests_post:
            with mock.patch("backend.api.routes.auth.requests.get") as requests_get:
    ...
    def test_reset_password(self):
        ...
        rv = self.client.post("/api/auth/password-reset/confirm", json={"token": token + "x", "new_password": "new-pass"})
        self.assertEqual(rv.status_code, 400)

        rv = self.client.post("/api/auth/password-reset/confirm", json={"token": token, "new_password": "new-pass"})
        self.assertEqual(rv.status_code, 204)

        rv = self.client.post("/api/auth/tokens", auth=(TEST_USER["username"], TEST_USER["password"]))
        self.assertEqual(rv.status_code, 401)

        rv = self.client.post("/api/auth/tokens", auth=(TEST_USER["username"], "new-pass"))
        self.assertEqual(rv.status_code, 200)
