...
class APIFairy:
    ...
    def init_app(self, app):
        ...
        self.ui = app.config.get("APIFAIRY_UI", "elements")
        ...
        self.apispec_decorators = app.config.get("APIFAIRY_APISPEC_DECORATORS", [])
        self.apispec_path = app.config.get("APIFAIRY_APISPEC_PATH", "/apispec.json")
        ...
        if self.ui_path:
            def docs():
                ...
                return render_template(f"apifairy/{self.ui}.html", title=self.title, version=self.version)
        ...
        def http_error(error):
            return self.error_handler_callback(error.status_code, error.messages)
    ...
    @staticmethod
    def default_error_handler(status_code, messages):
    ...
    def _generate_apispec(self):
        ...
        ma_plugin.converter.field_mapping[fields.AbsoluteURLFor] = ("string", "url")
        ma_plugin.converter.field_mapping[sqla.HyperlinkRelated] = ("string", "url")

        # Configure FileField
        ma_plugin.converter.field_mapping[apifairy_fields.FileField] = ("string", "binary")
            ...
            for method in methods:
                ...
                operation = {
                    ...
                    "parameters": [
                        ...
                        for schema, location in view_func._spec.get('args', [])if location != "body"
                    ],
                }
                if tag:
                    operation["tags"] = [tag]
                docs = [line.strip() for line in (view_func.__doc__ or "").strip().split("\n")]
                if docs[0]:
                ...
                if view_func._spec.get("response"):
                    ...
                    response_schema = view_func._spec.get("response")
                    if isinstance(response_schema, OneOfSchema):
                        ...
                        operation["responses"] = {
                            code: {
                                "content": {
                                    "application/json": {
                                        "schema": {
                                            "oneOf": list(response_schema.type_schemas.values())
                                        }
                                    }
                                }
                            }
                    ...
                    operation['responses'][code]['description'] = (view_func._spec['description']
                                                                   or HTTP_STATUS_CODES[int(code)])
                    if view_func._spec.get('response_headers'):
                        schema = view_func._spec.get('response_headers')
                        if isinstance(schema, type):
                            schema = schema()
                        headers = ma_plugin.converter.schema2parameters(schema, location='headers')
                        operation['responses'][code]['headers'] = {header['name']: header for header in headers}
                else:
                    operation['responses'] = {'204': {'description': HTTP_STATUS_CODES[204]}}

                if view_func._spec.get('other_responses'):
                    for status_code, response in view_func._spec.get('other_responses').items():
                        ...
                        if 'description' not in operation['responses'][status_code]:
                            operation['responses'][status_code]['description'] = HTTP_STATUS_CODES[int(status_code)]
                    ...
                    if media_type is None and location == "form":
                        ...
                        for field in schema.dump_fields.values():
                            if isinstance(field, apifairy_fields.FileField):
                                has_file = True
                                break
                        media_type = "application/x-www-form-urlencoded" if not has_file else "multipart/form-data"
                ...
                if view_func._spec.get('auth'):
                    operation['security'] = [{security[view_func._spec['auth']]: view_func._spec['roles']}]
            ...
            if is_endpoint:
                ...
                if path_arguments:
                    ...
                    for _, type_, name in path_arguments:
                        ...
                        if annotations.get(name) is int:
                            argument["schema"] = {"type": "integer"}
                        elif annotations.get(name) is MediaType:
                            argument["schema"]["enum"] = [media_type.value for media_type in MediaType]
                        elif annotations.get(name) is JobType:
                            argument["schema"]["enum"] = [job_type.value for job_type in JobType]
