...
def response(schema, status_code=200, description=None, headers=None):
    ...
    def decorator(f):
        f = _ensure_sync(f)
        _annotate(f, response=schema, status_code=status_code, description=description, response_headers=headers)
        ...
        def _response(*args, **kwargs):
            ...
            if isinstance(rv, tuple):
                ...
                if isinstance(schema, OneOfSchema):
                    return jsonify(schema.dump(rv)), status_code
                else:
