...
function StatsPage() {
    ...
    const apiData: StatsApiData = useSuspenseQuery(statsOptions(mediaType, username)).data;
    ...
    const addComparison = async (username: string) => {
...
const NavigationButtons = ({settings}) => {
    const popRef = useRef<HTMLButtonElement>();
    const {mediaType, username} = Route.useParams();
