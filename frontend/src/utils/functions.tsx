...
import {MediaStatus, MediaType, TierDifficulty} from "@/utils/types.tsx";
...
export const getFeelingValues = (size: number = 20): Array<{ value: number | null; icon: ReactNode | "--" }> => {
    return [
        {value: null, icon: "--"},
...
export const genreListsToListsOfDict = (stringList: Array<string>): Array<{ value: string, label: string }> => {
...
export const sliceIntoParts = (arr: Array<any>, n: number) => {
...
export const getLangCountryName = (name: string, type: "region" | "language") => {
...
export const getMediaColor = (media: MediaType) => {
    const colors = {
        series: "#216e7d",
        anime: "#945141",
        movies: "#8c7821",
...
export const getLevelColor = (intLevel: number) => {
...
export const globalStatsTimeFormat = (minutes: number): string => {
...
export const formatMinutes = (minutes: number, options?: { onlyHours: boolean }) => {
...
export const formatDateTime = (dateInput, options?: { useLocalTz?: boolean, onlyYear?: boolean, includeTime?: boolean }) => {
    if (!dateInput) {
        return "--";
    }
...
export function downloadFile(data: string, filename: string, mimeType: "text/csv" | "application/json") {
...
export function jsonToCsv(items: Array<any>) {
...
export const diffColors = (difficulty?: TierDifficulty, bg?: boolean = false) => {
