...
export const AchievementSummary = ({ summary }: AchievementSummaryProps) => {
    return (
        <div className="grid grid-cols-5 gap-6 w-[80%] mx-auto max-lg:w-[95%] max-sm:grid-cols-3
        max-sm:w-full max-sm:gap-2 max-sm:mb-4">
            {summary.map((diff) =>
                <Card key={diff.tier}>
                    <CardContent className="p-2 px-0 max-sm:p-3">
                        <div className="flex items-center justify-center font-medium">
                            <div key={diff.tier} className="flex items-center gap-4 max-sm:gap-2">
