...
import {TmdbTrendingTvResponse, TmdbTvDetails} from "@/lib/server/types/provider.types";
...
export class TvProviderService extends BaseTrendsProviderService<TvRepository> {
    ...
    protected async _getMediaIdsForBulkRefresh() {
        return this.client.getTvChangedIds().then(changedApiIds =>
            this.repository.getMediaIdsToBeRefreshed(changedApiIds)
        );
    }
