...
interface ProfileTabHeaderProps<T extends string> {
    mediaTypes: T[];
    activeTab: T | "overview";
    setActiveTab: (value: T | "overview") => void;
}


export const ProfileTabHeader = <T extends MediaType>({ activeTab, setActiveTab, mediaTypes }: ProfileTabHeaderProps<T>) => {
    const tabs: { label: T | "overview"; icon: React.ReactNode }[] = [
        { label: "overview", icon: <BarChart3 className="size-4"/> },
